...
export const collections = {
  ...
  analytics: null as Collection<MongoTypes.Analytics>,
...
onConnect(async (db) => {
  ...
  collections.analytics = db.collection("analyticsCollection")
      ...
      case "_webhookTimestamp":
          res.push([
            ...
            timestamp,
            {
              object: [timestamp, 0],
              writable: [timestamp, 0],
              // disabling type verification because i actually want v to be a Date
              value: [timestamp, [v as any, "xsd:dateTime"]],
            },
          ]);
        break;
      default:
...
export async function saveAnalyticsTimestamp(
  ...
  void collections.analytics.updateOne(
    { _id: id },
    updateValues,
    { upsert: true }
  );
...
export async function getAnalyticsTimestamp(id: string): Promise<number> {
  const queryResult = await collections.analytics.findOne({ _id: id })
  if(queryResult === null)
    return null;
